function New-RSAKeyPair {
    ...
    Enter the path to save the key to (Default: C:\Users\nate\.ssh\id_rsa): .\Testing\id_NerdCrypt
    Enter desired key length (Default: 4096):
    Enter passphrase (Default: No passphrase):
    Saving private key to path    : .\Testing\id_NerdCrypt
    Saving public SSH key to path : .\Testing\id_NerdCrypt.pub
    Saving public pem key to path : .\Testing\id_NerdCrypt.pem
    ...
    ssh-rsa AAAAB3NzaC1yc2EAAAABAwAAAIEAo2CDoZRSy7JDJbX3ygsj3L09rMxq+46lMkWv6K33Cng3y4DokqqyUc2KCzhspBViGzVl3mJ+Y4S9O+D4bktcSDRZbEmZ0cVsFZFEAI17iEKnZHZnaqMIoIzaK2TS0rnQbkYpSDfKUAZtwSNiWB0TfMFdnOY6UJdlfLGzPeFJWTU= NerdCrypt@User@Computer
    ...
    At E:\Git\NerdCrypt\BuildOutput\NerdCrypt\0.2.0\NerdCrypt.psm1:177 char:21
    ...
    [OutputType('alainQtec.NerdCrypt.RSAKey')]
    [CmdletBinding()]
    Param(
    ...
    Process {
        if ($Interactive) {
            ...
            if (-not ([System.String]::IsNullOrEmpty([xconvert]::Tostring($Password)))) {
                $confirmed = Read-Host -AsSecureString -Prompt "Enter the same passphrase to confirm"
                if ([xconvert]::Tostring($confirmed) -ne ([xconvert]::Tostring($Password))) {
                    Write-Error "Passphrases provided do not match! Exiting"
                    return
                }
                Write-Host "Generating passphrase protected key pair"
                $keys = [alainQtec.NerdCrypt.RSA]::Generate($Length, [xconvert]::Tostring($Password))
            } else {
                Write-Host "Generating key pair"
                $keys = [alainQtec.NerdCrypt.RSA]::Generate($Length)
            }
            if (-not $NoFile) {
            ...
            if (-not $NoFile -and -not $Force -and (Test-Path $Path)) {
                ...
                $keys = if ($PSBoundParameters.ContainsKey('Password')) {
                    Write-Host "Generating passphrase protected key pair"
                    [alainQtec.NerdCrypt.RSA]::Generate(
                        $Length,
                        $(
                            if ($Password -is [SecureString]) {
                                [xconvert]::Tostring($Password)
                            } else {
                                "$Password"
                            }
                        )
