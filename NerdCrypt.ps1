...
class K3Y {
    ...
    [string]GetK3YUCode() {
        return [K3Y]::GetK3YUCode($this.User.Password, $this.Expirity.Date, $(Get-Random ([Enum]::GetNames('Compression' -as 'Type'))), $this._PID)
    }
    [string]static GetK3YUCode([securestring]$Password, [datetime]$Expirity, [string]$Compression, [int]$_PID) {
    ...
    [void]hidden SetK3YUID() {
        $this.UID = [securestring][xconvert]::ToSecurestring($this.GetK3YUCode());
    }
    [void]hidden SetK3YUID([securestring]$Password, [datetime]$Expirity, [string]$Compression, [int]$_PID) {
        ...
        $this.UID = [securestring][xconvert]::ToSecurestring([string][K3Y]::GetK3YUCode($Password, $Expirity, $Compression, $_PID));
    }
